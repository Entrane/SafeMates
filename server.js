...
// 3b. Compression GZIP/Brotli pour améliorer les performances Lighthouse
app.use(compression({
    filter: (req, res) => {
...
// Route pour la page de modération
...
// ====================================================
// SYSTÈME DE REPORTS ET MODÉRATION
// ====================================================
...
function isAdmin(req, res, next) {
    db.get('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, user) => {
        if (err || !user || !user.is_admin) {
            return res.status(403).json({ error: 'access_denied', message: 'Accès réservé aux administrateurs' });
        }
        next();
    });
}
...
app.post('/api/reports', isAuthenticated, (req, res) => {
    ...
    // Trouver l'ID de l'utilisateur signalé
    db.get('SELECT id FROM users WHERE username = ?', [reportedUsername], (err, reported) => {
        ...
        if (reporterId === reportedId) {
            return res.status(400).json({ error: 'cannot_report_self', message: 'Vous ne pouvez pas vous signaler vous-même' });
        }
                ...
                if (!err && admins) {
                    admins.forEach(admin => {
                        createNotification(
                            ...
                            'new_report',
                            'Nouveau signalement',
                            `${req.user.username} a signalé ${reportedUsername} pour ${reason}`,
                            reportedId
                        );
                    });
                }
...
// POST /api/reports/:id/resolve - Résoudre un report (admin uniquement)
app.post('/api/reports/:id/resolve', isAuthenticated, isAdmin, (req, res) => {
    ...
    `, [adminId, note || '', reportId], function(err) {
        if (err) {
            console.error('Erreur résolution report:', err);
            return res.status(500).json({ error: 'server_error' });
        }
...
app.post('/api/sanctions', isAuthenticated, isAdmin, (req, res) => {
    ...
    db.get('SELECT id FROM users WHERE username = ?', [username], (err, user) => {
        ...
        // Si c'est un ban permanent, mettre à jour is_banned
        ...
        `, [userId, type, reason, durationHours || null, adminId, expiresAt], function(err) {
            ...
            if (type === 'warning') {
                ...
                title = 'Compte banni';
                message = `Votre compte a été banni: ${reason}`;
            }
            ...
            res.json({
                success: true,
                message: `Sanction ${type} appliquée à ${username}`,
...
app.get('/api/sanctions/:username', isAuthenticated, isAdmin, (req, res) => {
    ...
    db.get('SELECT id FROM users WHERE username = ?', [username], (err, user) => {
        ...
        `, [user.id], (err, sanctions) => {
            if (err) {
                console.error('Erreur récupération sanctions:', err);
                return res.status(500).json({ error: 'server_error' });
            }
            res.json(sanctions);
        });
...
// GET /api/moderation/stats - Statistiques de modération (admin uniquement)
...
app.get('/api/moderation/banned-users', isAuthenticated, isAdmin, (req, res) => {
    ...
    db.all(sql, [], (err, users) => {
        if (err) {
            console.error('Erreur récupération utilisateurs bannis:', err);
            return res.status(500).json({ error: 'server_error' });
        }
        res.json(users);
    });
...
// POST /api/moderation/unban/:userId - Débannir un utilisateur (admin uniquement)
app.post('/api/moderation/unban/:userId', isAuthenticated, isAdmin, (req, res) => {
    ...
    db.get('SELECT id, username, is_banned FROM users WHERE id = ?', [userId], (err, user) => {
        ...
        db.run('UPDATE users SET is_banned = 0 WHERE id = ?', [userId], function(err) {
            ...
            // Désactiver toutes les sanctions actives de type 'ban'
            db.run('UPDATE sanctions SET is_active = 0 WHERE user_id = ? AND type = "ban" AND is_active = 1', [userId], (err) => {
                if (err) {
                    console.error('Erreur désactivation sanctions:', err);
                }
            });
            ...
            createNotification(
                ...
                'account_update',
                'Compte débanni',
                'Votre compte a été débanni. Vous pouvez à nouveau accéder à MatchMates.',
                adminId
            );

            logger.info(`Utilisateur ${user.username} (ID: ${userId}) débanni par admin ID: ${adminId}`);
